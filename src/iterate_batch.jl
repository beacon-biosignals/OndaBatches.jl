...
Base.@kwdef struct RandomBatches
    ...
    function RandomBatches(labeled_signals,
                           ...
                           batch_size,
                           batch_duration)
        Legolas.validate(labeled_signals, Legolas.Schema("labeled.signal@1"))
...
[`"batch-item@1"`](@ref BatchItem) schema.
...
[`"batch-item@1"`](@ref BatchItem), and are consumed by
...
function iterate_batch_item(batches::RandomBatches, rng)
    ...
    batch_item = Tables.rowmerge(sub_label_span(signal_label_row,
                                                batch_label_span);
                                 batch_channels)
    return BatchItem(batch_item), rng
end
